        ...
        for (int z = 0; z < World.chunkSize; z++)
            for (int y = 0; y < World.chunkSize; y++)
                for (int x = 0; x < World.chunkSize; x++)
                {
                    ...
                    if (Utils.fBM3D(worldX, worldY, worldZ, 0.1f, 3, 0.5f) < 0.42f) // make dirt
                    {
                        chunkData[x, y, z] = new Block(Block.BlockType.AIR, pos, chunk.gameObject, this);
                    }
                    else if (worldY <= Utils.GenerateStoneHeight(worldX, worldZ)) // make dirt
                    {
                        if (Utils.fBM3D(worldX, worldY, worldZ, 0.1f, 2, 0.5f)< 0.4f && worldY < 40)
                        {
                    ...
                    else if (worldY == Utils.GenerateHeight(worldX, worldZ)) // grass
                    {
                        chunkData[x, y, z] = new Block(Block.BlockType.GRASS, pos, chunk.gameObject, this);
                    }
                    else if (worldY < Utils.GenerateHeight(worldX, worldZ)) // make stone
                    {

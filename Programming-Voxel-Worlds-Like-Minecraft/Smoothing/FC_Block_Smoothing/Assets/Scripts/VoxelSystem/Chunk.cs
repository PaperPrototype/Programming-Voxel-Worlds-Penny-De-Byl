...
public class Chunk
{
	...
	public Vector3[,,] allChunkVerts = new Vector3[World.chunkSize + 1, World.chunkHeight + 1, World.chunkSize + 1];
	...
	public void SmoothChunk(int chunkSize, int chunkHeight, float smooth, float extrude)
    {
		for (int x = 0; x < chunkSize; x++)
			for (int y = 0; y < chunkHeight; y++)
				for (int z = 0; z < chunkSize; z++)
				{
					chunkData[x, y, z].SmoothChunkVerts(smooth, extrude);
				}
	}
		...
		if (World.smoothing)
		{
			...
			for (int x = 0; x <= chunkSize; x++)
				for (int y = 0; y <= chunkHeight; y++)
					for (int z = 0; z <= chunkSize; z++)
					{
						allChunkVerts[x, y, z] = new Vector3(x, y, z);
					}
		}
